...
Recall that in \todonit{reference to definition of refuter} we only require
...
\todo{figure indexing wrt to how input is written in the tape}
...
\todopolish{check that coefficients are updated everywhere}
\begin{theorem}
...
(using a larger tape alphabet). \todowrite{Explain this better? In the introduction?} 
...
(for any even value of $n$ for which there is a counterexample, it finds one, and for big enough $n$ there are always counterexamples by \todonit{Reference to lower bound}).
\end{proof}
